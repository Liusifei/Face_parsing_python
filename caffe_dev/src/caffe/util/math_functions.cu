...
template <typename Dtype>
__global__ void scalar_max_kernel(const int n, const Dtype alpha,const Dtype* b,Dtype* y) {
  CUDA_KERNEL_LOOP(index, n) {
	  y[index] = alpha *(alpha > b[index]) + b[index]*(alpha <= b[index]);
  }
}
template <>
void caffe_gpu_scalar_max(const int N, const float alpha,const float *b, float* Y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
	scalar_max_kernel<float><<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(
      N, alpha,b, Y);
}

template <>
void caffe_gpu_scalar_max(const int N, const double alpha,const double *b, double* Y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
	scalar_max_kernel<double><<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(
      N, alpha,b, Y);
}
...
template <typename Dtype>
__global__ void scalar_min_kernel(const int n, const Dtype alpha,const Dtype* b,Dtype* y) {
  CUDA_KERNEL_LOOP(index, n) {
	  y[index] = alpha *(alpha < b[index]) + b[index]*(alpha >= b[index]);
  }
}
template <>
void caffe_gpu_scalar_min(const int N, const float alpha,const float *b, float* Y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
	scalar_min_kernel<float><<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(
      N, alpha,b, Y);
}

template <>
void caffe_gpu_scalar_min(const int N, const double alpha,const double *b, double* Y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
	scalar_min_kernel<double><<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(
      N, alpha,b, Y);
}


template <typename Dtype>
__global__ void get_gpu_dist_matrix(const int n,const Dtype * inputdata, const int feaNum, const int feaDim, Dtype * distmatrix) {
  CUDA_KERNEL_LOOP(index, n) {
	int r=index/feaNum;
	int c=index%feaNum;
	int i=0;
	...
	if(r==c)
		...
		for(i=0;i<feaDim;i++)
		{
			d = inputdata[r*feaDim + i] - inputdata[c*feaDim + i];
			dist += d*d;
		}
...
template <typename Dtype>
void Get_dist_matrix_gpu(const Dtype * inputdata, const int feaNum, const int feaDim, Dtype * distmatrix)
{//return eculidean distance martix, liangji20040249@gmail.com, 20160426
	

	int count = feaNum* feaNum;
	get_gpu_dist_matrix<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count,inputdata,feaNum,feaDim,distmatrix);


	CUDA_POST_KERNEL_CHECK;
	return;
}
template void Get_dist_matrix_gpu<float>(const float * inputdata, const int feaNum, const int feaDim, float * distmatrix);
template void Get_dist_matrix_gpu<double>(const double * inputdata, const int feaNum, const int feaDim, double * distmatrix);
