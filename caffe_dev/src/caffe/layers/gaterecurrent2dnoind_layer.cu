...
__device__ void get_gate_idx_sf(int h1,int w1,int h2,int w2, int * out,bool horizontal, bool reverse)
{
...
__device__ Dtype get_data_sf(Dtype * data, int num, int channels,int height, int width,int n,int c,int h,int w)
{
	if(h<0 || h >=height)
		return 0;
	if(w<0 || w >= width)
...
template <typename Dtype>
__device__ void set_data_sf(Dtype * data, int num, int channels,int height, int width,int n,int c,int h,int w,Dtype v)
{
	if(h<0 || h >=height)
		return ;
	if(w<0 || w >= width)
...
template <typename Dtype>
__device__ Dtype get_gate_sf(Dtype * data, int num, int channels,int height, int width,int n,int c,int h1,int w1,int h2,int w2,bool horizontal,bool reverse)
{
	if(h1<0 || h1 >=height)
		return 0;
	if(w1<0 || w1 >= width)
...
template <typename Dtype>
__device__ void set_gate_sf(Dtype * data, int num, int channels,int height, int width,int n,int c,int h1,int w1,int h2,int w2,bool horizontal,bool reverse,Dtype v)
{
	if(h1<0 || h1 >=height)
		return ;
	if(w1<0 || w1 >= width)
...
template <typename Dtype>
__device__ void set_gate_add_sf(Dtype * data, int num, int channels,int height, int width,int n,int c,int h1,int w1,int h2,int w2,bool horizontal,bool reverse,Dtype v)
{
	if(h1<0 || h1 >=height)
		return ;
	if(w1<0 || w1 >= width)
...
CUDA_KERNEL_LOOP(index, count) {
	...
	Dtype g_data_1 = get_gate_sf(G1,num,channels,height,width,n,c,h,w,h-1,w-1,horizontal,reverse);
	Dtype h_minus1_data_1 = get_data_sf(H,num,channels,height,width,n,c,h-1,w-1);
	Dtype h1_minus1 = g_data_1 * h_minus1_data_1;

	Dtype g_data_2 = get_gate_sf(G2,num,channels,height,width,n,c,h,w,h,w-1,horizontal,reverse);
	Dtype h_minus1_data_2 = get_data_sf(H,num,channels,height,width,n,c,h,w-1);
	Dtype h2_minus1 = g_data_2 * h_minus1_data_2;

	Dtype g_data_3 = get_gate_sf(G3,num,channels,height,width,n,c,h,w,h+1,w-1,horizontal,reverse);
	Dtype h_minus1_data_3 = get_data_sf(H,num,channels,height,width,n,c,h+1,w-1);
...
CUDA_KERNEL_LOOP(index, count) {
	...
	Dtype g_data_2 = get_gate_sf(G2,num,channels,height,width,n,c,h,w,h-1,w,horizontal,reverse);
	Dtype h_minus1_data_2 = get_data_sf(H,num,channels,height,width,n,c,h-1,w);
	Dtype h2_minus1 = g_data_2 * h_minus1_data_2;
...
CUDA_KERNEL_LOOP(index, count) {
	...
	Dtype add1_h2_diff = get_data_sf(Hdiff,num,channels,height,width,n,c,h,w+1);
	Dtype add1_g2_data = get_gate_sf(G2,num,channels,height,width,n,c,h,w,h,w+1,horizontal,reverse);
	...
	Dtype g2_data =  get_gate_sf(G2,num,channels,height,width,n,c,h,w,h,w-1,horizontal,reverse);
	...
	Dtype h2_minus1_data = get_data_sf(H,num,channels,height,width,n,c,h,w-1);
...
//modified
template <typename Dtype>
void GateRecurrent2dnoindLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
//modified
template <typename Dtype>
void GateRecurrent2dnoindLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(GateRecurrent2dnoindLayer);
